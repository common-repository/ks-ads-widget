...
(function($){
	...
	imageWidget = {
		...
		sendToEditor : function(h) {
			...
			// change button text
			$('#add_ads-widget-'+self.IW_instance+'-image').html($('#add_ads-widget-'+self.IW_instance+'-image').html().replace(/Add Image/g, 'Change Image'));
		},
	...
	$(document).ready(function() {
		...
		if ( typeof $.fn.on !== 'undefined' ) {
			$("#wpbody").on("click", ".thickbox-ads-widget", imageWidget.imgHandler);
		}
		else {
			$("a.thickbox-ads-widget").live('click', imageWidget.imgHandler);
		}

		// Modify thickbox link to fit window. Adapted from wp-admin\js\media-upload.dev.js.
		$('a.thickbox-ads-widget').each( function() {
