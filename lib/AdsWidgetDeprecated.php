...
class AdsWidgetDeprecated {
	...
	function admin_setup() {
		...
		if ( 'widgets.php' == $pagenow ) {
			...
			wp_enqueue_script( 'ks-ads-widget', plugins_url('resources/js/ads-widget.deprecated.js', dirname(__FILE__)), array('thickbox'), KS_Ads_Widget::VERSION, TRUE );
		}
		elseif ( 'media-upload.php' == $pagenow || 'async-upload.php' == $pagenow ) {
			wp_enqueue_script( 'ks-ads-widget-fix-uploader', plugins_url('resources/js/ads-widget.deprecated.upload-fixer.js', dirname(__FILE__)), array('jquery'), KS_Ads_Widget::VERSION, TRUE );
			add_filter( 'image_send_to_editor', array( $this,'image_send_to_editor'), 1, 8 );
			add_filter( 'gettext', array( $this, 'replace_text_in_thickbox' ), 1, 3 );
			add_filter( 'media_upload_tabs', array( $this, 'media_upload_tabs' ) );
			add_filter( 'ads_widget_image_url', array( $this, 'https_cleanup' ) );
		}
	 ...
	 *
	 * @author King Soft (Peter Chester)
	 */
	...
	function replace_text_in_thickbox($translated_text, $source_text, $domain) {
		if ( $this->is_sp_widget_context() ) {
			if ('Insert into Post' == $source_text) {
				return __('Insert Into Widget', 'ads_widget' );
			}
		}
